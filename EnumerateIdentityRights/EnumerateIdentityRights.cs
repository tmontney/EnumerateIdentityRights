...
namespace EnumerateIdentityRights
{
    ...
    [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Unicode)]
    struct LSA_UNICODE_STRING_WITHBUFFER
    {
        internal ushort Length;
        internal ushort MaximumLength;
    ...
    public sealed class LsaWrapper : IDisposable
    {
        ...
        const uint STATUS_OK = 0;
        const uint STATUS_ACCESS_DENIED = 0xc0000022;
        const uint STATUS_INSUFFICIENT_RESOURCES = 0xc000009a;
        const uint STATUS_NO_MEMORY = 0xc0000017;
        const uint STATUS_NO_RIGHTS = 0xc0000034;
        ...
        public string[] GetPrivileges(string account)
        {
            ...
            IntPtr rightsPtr;
            int countOfRights;
            uint ret = Win32Sec.LsaEnumerateAccountRights(lsaHandle, pSid, out rightsPtr, out countOfRights);

            if (ret == STATUS_OK)
            {
                LSA_UNICODE_STRING_WITHBUFFER lsaUserRight;
                long pointerValue = rightsPtr.ToInt64();

                for (int i = 0; i < countOfRights; i++)
                {
                    lsaUserRight = (LSA_UNICODE_STRING_WITHBUFFER)Marshal.PtrToStructure(new IntPtr(pointerValue), typeof(LSA_UNICODE_STRING_WITHBUFFER));
                    string userRight = Marshal.PtrToStringAuto(lsaUserRight.Buffer);
                    privileges.Add(userRight);
                    pointerValue += Marshal.SizeOf(lsaUserRight);
                }

                return privileges.ToArray();
            }
            if( ret == STATUS_NO_RIGHTS)
            {
                return privileges.ToArray();
            }
            if (ret == STATUS_ACCESS_DENIED)
